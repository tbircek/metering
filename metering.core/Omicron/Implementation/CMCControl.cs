            ...
            try
            {
                ...
                for (testStartValue = From; testStartValue <= (To + Delta * 1 / 1000); testStartValue += Delta)
                {
                    ...
                    if (!IoC.Commands.Token.IsCancellationRequested)
                    {
                        ...
                        // debug purposes, will bypass this subroutine
                        if (!IoC.TestDetails.Register.Contains(","))
                        ...
                        // debug purposes, Register has at least one comma
                        else
                            // set timer to read modbus register per the user specified time.
                            MdbusTimer = new Timer(IoC.Modbus.MeasurementIntervalWithCommaCallbackAsync, IoC.TestDetails.Register, TimeSpan.FromSeconds(Convert.ToDouble(IoC.TestDetails.StartMeasurementDelay)), TimeSpan.FromMilliseconds(Convert.ToDouble(IoC.TestDetails.MeasurementInterval)));
